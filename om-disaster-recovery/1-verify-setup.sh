...
echo "-----------------------------"
check "Primary OM appDB accessible on 27018" "docker exec mongodb-ops-manager mongosh --quiet --eval 'db.adminCommand({ping: 1})' 2>&1 | grep -q 'ok: 1'"
check "Primary OM appDB container running" "docker ps | grep -q 'mongodb-ops-manager'"

echo ""
echo "3. Checking Replica Set Configuration"
echo "--------------------------------------"
RS_NAME=$(docker exec mongodb-ops-manager mongosh --quiet --eval "rs.status().set" 2>/dev/null || echo "")
if [ -n "$RS_NAME" ]; then
...
RS_MEMBER=$(docker exec mongodb-ops-manager mongosh --quiet --eval "rs.status().members[0].name" 2>/dev/null || echo "")
if [ -n "$RS_MEMBER" ]; then
...
echo "---------------------------------------"
DBS=$(docker exec mongodb-ops-manager mongosh --quiet --eval "db.adminCommand({listDatabases: 1}).databases.map(d => d.name).join(', ')" 2>/dev/null || echo "")
if [ -n "$DBS" ]; then
...
echo "-------------------------"
USER_COUNT=$(docker exec mongodb-ops-manager mongosh --quiet --eval "db.getSiblingDB('mmsdbconfig').users.countDocuments({})" 2>/dev/null || echo "0")
echo "Users in mmsdbconfig: $USER_COUNT"

PROJECT_COUNT=$(docker exec mongodb-ops-manager mongosh --quiet --eval "db.getSiblingDB('mmsdbconfig').groups.countDocuments({})" 2>/dev/null || echo "0")
